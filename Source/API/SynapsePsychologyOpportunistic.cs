...
Skills: {skillBonuses}{hometownContext}{childhoodContext}";
        ...
        private static string GetFactionHistoryContext(Faction faction)
        {
            ...
            {
                RimSynapse.SynapseLogger.Warn("psychology", $"[RimSynapse-Psychology] Could not read faction history from StoryTeller: {ex.Message}");
            }
            return "";
        }
                ...
                try
                {
                    ...
                    if (json != null)
                    {
                        ...
                        if (parsed != null && parsed.ContainsKey("Memory"))
                        {
                            ...
                            RimSynapse.SynapseLogger.Info("psychology", $"[RimSynapse-Psychology] Leader childhood memory generated for {leader.Name.ToStringShort} ({memoryText.Length} chars). Hometown: {coreComp.hometown ?? "none"}");
                        }
                    }
                }
                catch (Exception ex)
                {
                    RimSynapse.SynapseLogger.Warn("psychology", $"[RimSynapse-Psychology] Failed to parse leader childhood memory: {ex.Message}");
                }
            }
                ...
                try
                {
                    ...
                    if (json != null)
                    {
                        ...
                        if (parsed != null && parsed.ContainsKey("Memory"))
                        {
                            ...
                            RimSynapse.SynapseLogger.Info("psychology", $"[RimSynapse-Psychology] Leader rise memory generated for {leader.Name.ToStringShort} ({memoryText.Length} chars).");
                        }
                    }
                }
                catch (Exception ex)
                {
                    RimSynapse.SynapseLogger.Warn("psychology", $"[RimSynapse-Psychology] Failed to parse leader rise memory: {ex.Message}");
                }
            }
                ...
                try
                {
                    ...
                    if (json != null)
                    {
                        ...
                        if (parsed != null)
                        {
                            ...
                            RimSynapse.SynapseLogger.Info("psychology", $"[RimSynapse-Psychology] Leader personality profile synthesized for {leader.Name.ToStringShort}.");
                        }
                    }
                }
                catch (Exception ex)
                {
                    RimSynapse.SynapseLogger.Warn("psychology", $"[RimSynapse-Psychology] Failed to parse leader personality profile: {ex.Message}");
                }
            }

            // Finalize — mark backstory complete
            MarkBackstoryCreated(leader);
            RimSynapse.SynapseLogger.Info("psychology", $"[RimSynapse-Psychology] Leader backstory pipeline complete for {leader.Name.ToStringShort} (3-step).");
        }
